...
export const putDb = async (content) => {
  ...
  const OPENED_DB = await openDB(DB_NAME, 1);

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  const UNBOXING_DB = OPENED_DB.transaction(DB_NAME, 'readwrite');

  // TODO: Now create a variable for the store
  const DB_STORE = UNBOXING_DB.objectStore(DB_NAME);

  const request = DB_STORE.put({ id: 1, value: content });
...
export const getDb = async () => {
  ...
  const OPENED_DB = await openDB(DB_NAME, 1);
  const UNBOXING_DB = OPENED_DB.transaction(DB_NAME, 'readonly');
  const DB_STORE = UNBOXING_DB.objectStore(DB_NAME);
  // future todo: better way to refer/reuse? 
